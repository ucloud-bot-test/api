...
| **BackupType** | integer | 备份类型,取值为0或1，0表示自动，1表示手动 |No|
| **BeginTime** | integer | 过滤条件:起始时间(Unix时间戳) |No|
| **EndTime** | integer | 过滤条件:结束时间(Unix时间戳) |No|
...
| **TotalCount** | integer | 满足条件备份总数，如果指定dbid，则是该db备份总数 |No|
...
| **BackupSize** | integer | 备份文件大小(字节) |No|
| **BackupType** | integer | 备份类型,取值为0或1,0表示自动，1表示手动 |No|
...
| **BackupEndTime** | integer | 备份完成时间(Unix时间戳) |No|
