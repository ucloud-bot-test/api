...
| **ProtocolVersion** | integer | 请求协议版本，建议升级为1，为1时DiskType与UHost磁盘类型定义一致；默认为0 |No|
...
| **SnapshotCount** | integer | 该盘快照个数 |No|
| **SnapshotLimit** | integer | 该盘快照上限 |No|
...
| **CloneEnable** | integer | 是否支持克隆，1支持 ，0不支持 |No|
| **SnapEnable** | integer | 是否支持快照，1支持 ，0不支持 |No|
| **ArkSwitchEnable** | integer | 是否支持开启方舟，1支持 ，0不支持 |No|
