...
| **Limit** | integer | 返回数据长度，如果制定了Offset，则默认20，否则默认全部，非负整数 |No|
...
| 字段名 | 类型 | 描述信息 | 必填 |
|:---|:---|:---|:---|
| **TotalCount** | integer | 满足条件的域名个数 |No|
| **MaxDomainNum** | integer | 最大域名数量，默认20 |No|
| **ChargeType** | integer | 当前计费方式，10-流量付费 20-带宽日峰值 30-月95计费，31-月日均峰值， 32-月第四峰值 33-日均峰值之和 34- 日95再取平均 40-未选择计费方式 |No|
| **LastChargeType** | integer | 表示最后一次切换的计费方式，10=流量付费 20=带宽日峰值  30=按月后付费  40=未选择计费方式 |No|
...
| **ReferType** | integer | 0白名单，1黑名单 |No|
...
| **CreateTime** | integer | 域名创建的时间。格式：时间戳 |No|
| **ValidTime** | integer | 开始分配Cname时间。格式：时间戳 |No|
...
| **FollowOriginRule** | integer | 是否优先遵循源站头部缓存策略，0为不优先遵循源站，1为优先遵循源站缓存头部。默认为0 |No|
