...
| 字段名 | 类型 | 描述信息 | 必填 |
|:---|:---|:---|:---|
| **TotalCount** | integer | 满足条件的ULB总数 |No|
...
| **BandwidthType** | integer | 带宽类型，枚举值为： 0，非共享带宽； 1，共享带宽 |No|
| **Bandwidth** | integer | 带宽 |No|
| **CreateTime** | integer | ULB的创建时间，格式为Unix Timestamp |No|
| **ExpireTime** | integer | ULB的到期时间，格式为Unix Timestamp |No|
...
| **EnableLog** | integer | ULB是否开启日志功能。0，关闭；1，开启 |No|
...
| **BandwidthType** | integer | 弹性IP的带宽类型，枚举值：1 表示是共享带宽，0 普通带宽类型（暂未对外开放） |No|
| **Bandwidth** | integer | 弹性IP的带宽值（暂未对外开放） |No|
...
| **FrontendPort** | integer | VServer服务端口 |No|
...
| **ClientTimeout** | integer | 空闲连接的回收时间，单位：秒。 |No|
| **Status** | integer | VServer的运行状态。枚举值： 0 -> rs全部运行正常;1 -> rs全部运行异常；2 -> rs部分运行异常。 |No|
...
| **Port** | integer | 后端提供服务的端口 |No|
| **Enabled** | integer | 后端提供服务的实例启用与否，枚举值：0 禁用 1 启用 |No|
| **Status** | integer | 后端提供服务的实例运行状态，枚举值：0健康检查健康状态 1 健康检查异常 |No|
...
| **PolicyPriority** | integer | 内容转发优先级，范围[1,9999]，数字越大优先级越高。默认内容转发规则下为0。 |No|
| **VServerId** | string | 所属VServerId |No|
| **TotalCount** | integer | 默认内容转发类型下返回当前rs总数 |No|
